...
local logger = LrLogger('Provider')
logger:enable('print')
...
function Provider.signupDialog(propertyTable)
...
--            local cookies = {[1] = {field = 'Set-Cookie', value = 'yandex_login=svetlyak40wt'}}
            local cookies = Provider.auth(p.ya_login, p.ya_password)
            if cookies == nil then
...
function Provider.cookiesObserver(properties, key, newValue)
    if type(newValue) == 'table' and #newValue > 0 then
        ...
        LrFunctionContext.postAsyncTaskWithContext('get_albums', function(context)
            LrDialogs.attachErrorDialogToFunctionContext(context)
            Provider.get_albums(properties)
        end)
        ...
        properties.loginButtonAction = function(button)
            LrFunctionContext.callWithContext('signupDialog', function( context ) 
                LrDialogs.attachErrorDialogToFunctionContext(context)
                Provider.signupDialog(properties)
            end )
        end
...
function Provider.exportDialog(viewFactory, propertyTable)
    trace('Provider.exportDialog 2')
    local bind = LrView.bind
    local f = viewFactory
    ...
    Provider.dropCookiesIfNeeded(propertyTable)
    Provider.cookiesObserver(propertyTable, 'ya_cookies', propertyTable.ya_cookies)
    propertyTable:addObserver('ya_cookies', Provider.cookiesObserver)
...
function Provider.get_albums(p)
    local albums = {}
...
function Provider.upload(exportContext, path, photo)
    LrFunctionContext.callWithContext('signupDialog', function(context)
        LrDialogs.attachErrorDialogToFunctionContext(context)
...
function Provider.auth(login, password)
...
function Provider.postProcess(functionContext, exportContext)
    trace('Provider.postProcess')
    ...
    for i, rendition in exportContext:renditions{stopIfCanceled = true} do
        ...
        if success then
            Provider.upload(exportContext, pathOrMessage, photo)
        else
            error(pathOrMessage)
        end
...
function Provider.dropCookiesIfNeeded(propertyTable)
    local p = propertyTable
...
return {
    ...
    processRenderedPhotos = Provider.postProcess,
    sectionsForTopOfDialog = Provider.exportDialog,
}
